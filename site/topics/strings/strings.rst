...
.. note::
    ...
    just a variable name and is not required to be ``char`` because the things in the string are characters. I just
    chose ``char`` since it is an appropriate name for the variable. For example, the following code would work
    just fine:
